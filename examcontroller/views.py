...
class Allocateseat(View):
    ...
    def post(self,request):
        ...
        no_of_subjects=ExamDetails.objects.filter(exam_date=exam_date).all().count()
        ...
        no_of_subjects = len(exam_details)

        # Initializing the dictionary to store students per subject
        students_per_subject = {}
        subject_ids = []
        # Iterating over each exam detail to populate the dictionary
        ...
        for class_name in classrooms:
            try:
                # Fetch the classroom object based on the hallno
                classroom = Classroom.objects.get(id=class_name)

                # If classroom object is found, fetch capacity and columns
                capacity = classroom.capacity
                columns = classroom.columns

                # Append the total number of seats to the list
            ...
            print(non_null_classrooms)
            print(total_capacity)
            print(seats_per_classroom)
            print(columns_per_classroom)
            print(students_per_subject)
            print(int(total_classroom))
            classroom_seating = assign_seating(total_classroom, no_of_subjects, students_per_subject, columns_per_classroom,
                                               seats_per_classroom)

            # Print the seating arrangement for each classroom
            # Print the seating arrangement for each classroom
            if classroom_seating:
                for i, seating in enumerate(classroom_seating):
                    print(f"Classroom {i + 1}:")
                    for seat, subject in enumerate(seating):
                        print(f"Seat {seat + 1}: {subject}")
                    print()
            for i, seating in enumerate(classroom_seating):
                ...
                for seat, subject in enumerate(seating):
                    try:
                        # seat, subject = seat_subject
                        print(type(subject))
                        s = subject
                        first_element_stripped = s.split()

                        # Split the stripped string by whitespace
                        extracted_value = first_element_stripped[0]
                        print(extracted_value)
                        # Fetch the classroom object based on the hallno
                        subject_instance=Subject.objects.filter(subjectcode=extracted_value).first()
                        print(subject_instance)
                        # print(part_before_parentheses)
                        classroom = Classroom.objects.get(id=classrooms[i])
                        # Assuming classrooms is a list of classroom IDs
                        print(classroom)
                        # Create a new SeatingArrangement object and save it to the database
                        seating_entry = Seatingarrangement(classroom_number=classroom, seat_number=seat + 1,
                                                           subject=subject_instance, exam_date=exam_date)
                        seating_entry.save()
                    except Classroom.DoesNotExist:
                        # Handle the case where the classroom with the given ID does not exist
                        pass

            # Inputs
            cc=update_seating_arrangement_register_numbers()
